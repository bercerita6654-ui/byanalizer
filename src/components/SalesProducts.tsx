...
export default function SalesProducts() {
  ...
  // Margin & Profit Calculator States
  const [calcProductSku, setCalcProductSku] = useState<string>('manual');
  const [calcProductName, setCalcProductName] = useState<string>('');
  const [calcCogs, setCalcCogs] = useState<number>(0);
  ...
  // Sync calculator when selected product SKU changes
  useEffect(() => {
    if (calcProductSku === 'manual') {
      if (!calcSellingPrice) setCalcSellingPrice(150000);
      if (!calcCogs) setCalcCogs(100000);
      if (!calcProductName) setCalcProductName('Produk Kustom Baru');
      return;
    }

    const found = products.find(p => p.sku === calcProductSku);
    if (found) {
      setCalcProductName(found.name);
      const avgPrice = found.totalQty > 0 ? found.totalSales / found.totalQty : found.totalSales || 0;
      const roundedPrice = Math.round(avgPrice);
      setCalcSellingPrice(roundedPrice);
      setCalcVolume(found.totalQty || 100);

      // Check localStorage for saved COGS
      const savedCogs = localStorage.getItem(`product_cogs_${found.sku}`);
      if (savedCogs) {
        setCalcCogs(parseInt(savedCogs, 10));
      } else {
        // Default COGS is 70% of Selling Price (representing a solid 30% gross margin)
        setCalcCogs(Math.round(roundedPrice * 0.7));
      }
    }
  }, [calcProductSku, products]);

  const handleSaveCogs = () => {
    if (calcProductSku === 'manual') return;
    localStorage.setItem(`product_cogs_${calcProductSku}`, String(calcCogs));
    setSaveSuccess(true);
    setTimeout(() => setSaveSuccess(false), 2000);
  };

  // Unique list of products for dropdown selector
          ...
          <div id="margin-calculator-section" className="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-7 space-y-5">
                ...
                {calcProductSku === 'manual' && (
                  <div className="space-y-1.5">
                    <label className="text-[10.5px] font-black uppercase text-slate-400 tracking-wider">Nama Produk Kustom</label>
                    <input
                      type="text"
                      value={calcProductName}
                      onChange={(e) => setCalcProductName(e.target.value)}
                      placeholder="Masukkan nama produk kustom..."
                      className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all font-sans"
                    />
                  </div>
                )}

                {/* Input 1: Harga Modal (COGS) */}
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-[10.5px] font-black uppercase text-slate-400 tracking-wider flex items-center gap-1">
                      Harga Modal (COGS)
                      <span className="text-slate-300 font-normal">/ unit</span>
                    </label>
                    <span className="text-xs font-mono font-bold text-slate-500">{formatRupiah(calcCogs)}</span>
                  </div>
                  <div className="flex gap-3 items-center">
                    <input
                      type="number"
                      value={calcCogs || ''}
                      onChange={(e) => setCalcCogs(Math.max(0, parseInt(e.target.value, 10) || 0))}
                      placeholder="Masukkan Harga Modal"
                      className="w-32 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono font-bold text-slate-700 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                    />
                    <input
                      type="range"
                      min="0"
                      max={Math.max(500000, calcCogs * 2)}
                      step="500"
                      value={calcCogs}
                      onChange={(e) => setCalcCogs(parseInt(e.target.value, 10) || 0)}
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs font-mono font-bold text-indigo-600">{formatRupiah(calcSellingPrice)}</span>
                  </div>
                  <div className="flex gap-3 items-center">
                    <input
                      type="number"
                      value={calcSellingPrice || ''}
                      onChange={(e) => setCalcSellingPrice(Math.max(0, parseInt(e.target.value, 10) || 0))}
                      placeholder="Masukkan Harga Jual"
                      className="w-32 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono font-bold text-indigo-600 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                    />
                    <input
                      type="range"
                      min="0"
                      max={Math.max(1000000, calcSellingPrice * 2)}
                      step="500"
                      value={calcSellingPrice}
                      onChange={(e) => setCalcSellingPrice(parseInt(e.target.value, 10) || 0)}
                      className="flex-1 h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-indigo-600"
                    />
                  </div>
                </div>

                {/* Input 3: Volume Simulasi */}
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-[10.5px] font-black uppercase text-slate-400 tracking-wider flex items-center gap-1">
                      Volume Simulasi Penjualan
                      <span className="text-slate-300 font-normal">/ Qty</span>
                    </label>
                    <span className="text-xs font-mono font-bold text-slate-500">{formatNumberIndo(calcVolume)} pcs</span>
                  </div>
                  <div className="flex gap-3 items-center">
                    <input
                      type="number"
                      value={calcVolume || ''}
                      onChange={(e) => setCalcVolume(Math.max(1, parseInt(e.target.value, 10) || 1))}
                      placeholder="Volume"
                      className="w-32 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono font-bold text-slate-700 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                    />
                    <input
                      ...
                      step="10"
                      value={calcVolume}
                      onChange={(e) => setCalcVolume(parseInt(e.target.value, 10) || 1)}
                      className="flex-1 h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-indigo-600"
                    />
                  </div>
                </div>

                {/* Presets and Pricing Tools */}
                <div className="bg-slate-50/70 p-4 rounded-2xl border border-slate-100 space-y-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-black uppercase text-slate-600 tracking-wider">Pintasan Target Margin (Kalkulator Harga Jual Otomatis)</span>
                  </div>
                  <p className="text-[10px] text-slate-400 font-bold leading-normal">
                    Pilih target margin di bawah ini untuk menghitung harga jual yang disarankan secara otomatis berdasarkan Harga Modal (COGS) saat ini.
                  </p>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
                    {[15, 25, 35, 50].map((mPct) => (
                      <button
                        key={mPct}
                        type="button"
                        onClick={() => {
                          if (calcCogs > 0) {
                            const calculatedPrice = Math.round(calcCogs / (1 - mPct / 100));
                            setCalcSellingPrice(calculatedPrice);
                          }
                        }}
                        className="px-3 py-2 bg-white hover:bg-indigo-50 hover:border-indigo-200 border border-slate-200 rounded-xl text-[10px] font-black uppercase text-slate-600 hover:text-indigo-700 transition-all hover:scale-[1.02] cursor-pointer"
                      >
                        Margin {mPct}%
                      </button>
                    ))}
              ...
              {/* Right Column: Calculations & Bento Analytics (5 cols on lg) */}
              <div className="lg:col-span-5 bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-5 flex flex-col justify-between">
                
                {/* Header Information */}
                <div>
                  <div className="flex items-center justify-between border-b border-slate-200/60 pb-3">
                    <div className="truncate pr-2">
                      <span className="text-[9px] font-black uppercase bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded tracking-wider">Hasil Analisa</span>
                      <h4 className="text-xs font-black text-slate-800 uppercase tracking-wide truncate max-w-[200px] mt-1" title={calcProductName}>
                        {calcProductName || 'Produk Kustom'}
                      </h4>
                    </div>
                    {calcProductSku !== 'manual' && (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer border ${
                          saveSuccess
                            ...
                            : 'bg-white hover:bg-indigo-50 border-slate-200 text-slate-600 hover:border-indigo-200 hover:text-indigo-700'
                        }`}
                      >
                        <Check className={`w-3 h-3 ${saveSuccess ? 'block' : 'hidden'}`} />
                        {saveSuccess ? 'Tersimpan!' : 'Simpan Modal'}
                      </button>
                    )}
                  </div>
                  ...
                  {(() => {
                    const profitAmount = calcSellingPrice - calcCogs;
                    const profitMargin = calcSellingPrice > 0 ? (profitAmount / calcSellingPrice) * 100 : 0;
                    const markupPercent = calcCogs > 0 ? (profitAmount / calcCogs) * 100 : 0;
                    const totalRevenue = calcSellingPrice * calcVolume;
                    const totalProfit = profitAmount * calcVolume;

                    let badgeColor = "bg-rose-50 border-rose-200 text-rose-700";
                    let badgeLabel = "Rugi Kotor";
                    let badgeDesc = "Harga modal melebihi harga jual. Anda mengalami kerugian per unit!";
                    
                    if (profitAmount > 0) {
                      if (profitMargin < 10) {
                        badgeColor = "bg-amber-50 border-amber-200 text-amber-700";
                        badgeLabel = "Margin Tipis (Kurang Aman)";
                        badgeDesc = "Keuntungan sangat sensitif terhadap biaya operasional atau fluktuasi harga.";
                      } else if (profitMargin >= 10 && profitMargin < 30) {
                        badgeColor = "bg-emerald-50 border-emerald-200 text-emerald-700";
                        badgeLabel = "Margin Sehat (Standar Ritel)";
                        badgeDesc = "Tingkat profitabilitas normal dan aman untuk bisnis ritel / e-commerce umum.";
                      } else if (profitMargin >= 30 && profitMargin < 50) {
                        badgeColor = "bg-indigo-50 border-indigo-200 text-indigo-700";
                        badgeLabel = "Margin Tinggi (Menguntungkan)";
                        badgeDesc = "Tingkat profitabilitas yang sangat baik. Menunjukkan produk bernilai tambah tinggi.";
                      } else {
                        badgeColor = "bg-purple-50 border-purple-200 text-purple-700";
                        badgeLabel = "Margin Premium (Luar Biasa)";
                        badgeDesc = "Produk memiliki margin premium yang fantastis. Sangat menguntungkan bagi bisnis.";
                      }
                    }
                    ...
                    return (
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3 mt-4">
                          ...
                          {/* KPI 1: Profit per Unit */}
                          <div className="bg-white p-3.5 rounded-xl border border-slate-200/50 flex flex-col justify-between">
                            <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Profit / Unit</span>
                            <div className="mt-1">
                              <span className={`text-xs sm:text-sm font-mono font-black ${profitAmount >= 0 ? 'text-slate-800' : 'text-rose-600'}`}>
                                {formatRupiah(profitAmount)}
                              </span>
                            </div>
                          </div>

                          {/* KPI 2: Margin Keuntungan */}
                          <div className="bg-white p-3.5 rounded-xl border border-slate-200/50 flex flex-col justify-between">
                            <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Margin Profit</span>
                            <div className="mt-1 flex items-baseline gap-0.5">
                              <span className={`text-xs sm:text-sm font-mono font-black ${profitAmount >= 0 ? 'text-indigo-600' : 'text-rose-600'}`}>
                                {profitMargin.toFixed(1)}%
                              </span>
                            </div>
                          </div>

                          {/* KPI 3: Markup Percent */}
                          <div className="bg-white p-3.5 rounded-xl border border-slate-200/50 flex flex-col justify-between">
                            <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Markup COGS</span>
                            <div className="mt-1">
                              <span className={`text-xs font-mono font-black ${profitAmount >= 0 ? 'text-slate-700' : 'text-rose-600'}`}>
                                {markupPercent.toFixed(1)}%
                              </span>
                            </div>
                          </div>

                          {/* KPI 4: Total Expected Profit */}
                          <div className="bg-white p-3.5 rounded-xl border border-slate-200/50 flex flex-col justify-between">
                            <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider">Proyeksi Profit</span>
                            <div className="mt-1">
                              <span className={`text-xs font-mono font-black ${totalProfit >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                        ...
                        <div className="space-y-1.5 bg-white p-3.5 rounded-xl border border-slate-200/50">
                          <div className="flex justify-between text-[9px] font-black uppercase text-slate-400">
                            <span>Harga Modal ({Math.max(0, 100 - Math.max(0, profitMargin)).toFixed(0)}%)</span>
                            <span>Profit ({Math.max(0, profitMargin).toFixed(0)}%)</span>
                          </div>
                        ...
                        {/* Bottom Section: Badge Status & Advice */}
                        <div className={`p-4 rounded-xl border ${badgeColor} space-y-1.5 mt-4 sm:mt-0`}>
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] font-black uppercase tracking-wider bg-white/70 px-2 py-0.5 rounded-md shadow-sm">
                              {badgeLabel}
                            </span>
                          </div>
                      ...
                      paginatedProducts.map((p, idx) => {
                        ...
                        return (
                          <motion.tr 
                            ...
                            exit={{ opacity: 0 }}
                            transition={{ duration: 0.25, delay: Math.min(idx * 0.015, 0.15), ease: "easeOut" }}
                          ...
                          >
                            ...
                            <td className="py-3.5 px-4 text-center">
                              <button
                                type="button"
                                onClick={() => {
                                  setCalcProductSku(p.sku);
                                  document.getElementById('margin-calculator-section')?.scrollIntoView({ behavior: 'smooth' });
                                }}
                                className="inline-flex items-center gap-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200/50 hover:border-emerald-300 px-2 py-1 rounded-lg font-black text-[9px] uppercase tracking-wider cursor-pointer hover:scale-[1.03] active:scale-[0.97] transition-all"
                              >
